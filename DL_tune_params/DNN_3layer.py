...
def DL_tune(layer1, layer2, layer3):
    em_size = 50
    F_Doc2Vec = [str(i) for i in range(em_size)]
    F_gem = ['em ' + str(i + 1) for i in range(em_size)]

    print('3 layer tuning starts: total tests {0}'.format(str(len(layer1) * len(layer2) * len(layer3))))
    file1 = open("../data/3layer_aucs.csv", "w")
    file1.write("layer1\tlayer2\tlayer3\tauc\n")
    for i3 in range(len(layer3)):
        for i2 in range(len(layer2)):
            for i1 in range(len(layer1)):
                fpr_dl, tpr_dl = DL_Modeling(F_Doc2Vec + F_gem, [layer1[i1], layer2[i2], layer3[i3]], 1)
                file1.write("{0}\t{1}\t{2}\t{3}\n".format(str(layer1[i1]), str(layer2[i2]), str(layer3[i3]),
                                                          str(auc(fpr_dl, tpr_dl))))
    file1.close()
    print('3 layer tuning finished!')
